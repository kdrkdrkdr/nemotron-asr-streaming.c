...
static inline int32_t dot_i8_generic_inline(const int8_t *a, const int8_t *b, int n) {
    int32_t sum = 0;
    for (int i = 0; i < n; i++) {
...
static inline void dot4_i8_generic_inline(const int8_t *x,
...
void nemo_q8_matvec_fused_generic(float *y, const int8_t *x_q8, float x_scale,
    ...
    for (; o + 3 < out_dim; o += 4) {
        ...
        int32_t s0, s1, s2, s3;
        dot4_i8_generic_inline(x_q8, w0, w1, w2, w3, in_dim, &s0, &s1, &s2, &s3);
    ...
    for (; o < out_dim; o++) {
        int32_t acc = dot_i8_generic_inline(x_q8, w + (size_t)o * in_dim, in_dim);
        y[o] = (float)acc * x_scale * w_scales[o] + (b ? b[o] : 0.0f);
    }
}

int nemo_argmax_q8_range_generic(const int8_t *x_q8, float x_scale,
                                 const int8_t *w, const float *w_scales,
